...
"""
def _random_index_from_probabilities(P):
    assert(len(P) > 0)
...
class WordList:
    ...
    def generate_sentence(self):
        ...
        while next_index != len(self._words):  # while we shouldn't end the sentence
            words.append(self._words[next_index])
            next_index = self._generate_next_word_index(words[-1])

        return _prettify_sentence(words)

    def _generate_first_word_index(self):
        P = [word.p_start for word in self._words]

        return _random_index_from_probabilities(P)
